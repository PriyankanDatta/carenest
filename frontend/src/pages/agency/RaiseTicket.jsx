...
export default function AgencyRaiseTicket() {
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      navigate(`/agency/tickets/${data.id}`)
        ...
        <div className="card">
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex gap-3">
              <button type="button" onClick={() => navigate('/agency/tickets')} className="btn-secondary flex-1">Cancel</button>
              <button type="submit" disabled={busy} className="btn-primary flex-1">{busy ? 'Submitting…' : 'Submit'}</button>
            </div>
          </form>
        </div>
      </div>
